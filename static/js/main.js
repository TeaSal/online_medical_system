...
if (favDoctorsDiv) {
  fetch(`/api/favorites`)
    .then(res => res.json())
    .then(favs => {
      favDoctorsDiv.innerHTML = favs.map(f => `
        <div class="col-md-4">
          <div class="card p-3">
  ...
  // NAV: logout form handle (since layout uses form)
      ...
      doctorsListEl.innerHTML = docs.map(d => `
        <div class="col-12 col-md-6 col-lg-4">
          <div class="doctor-card">
            <img src="https://randomuser.me/api/portraits/med/men/${(d.id*7)%90}.jpg" width="64" height="64" style="border-radius:50%">
            <div style="flex:1">
  ...
  if (document.getElementById("confirmSched") || document.getElementById("schedTimeSlots")) {
    (function(){
      const parts = location.pathname.split("/").filter(Boolean);
      const docId = parseInt(parts[1]);
      document.getElementById("backToDoc").href = `/doctor/${docId}`;
      ...
      datePills.innerHTML = [...Array(7)].map((_,i) => {
        ...
        const day = d.getDate();
        return `<button class="date-pill btn btn-light" data-date="${d.toISOString().slice(0,10)}">${weekday}<br><strong>${day}</strong></button>`;
      }).join("");
      ...
      datePills.querySelectorAll(".date-pill").forEach(b => b.addEventListener("click", ()=>{
        datePills.querySelectorAll(".date-pill").forEach(x=>x.classList.remove("active"));
        b.classList.add("active"); chosenDate = b.dataset.date;
      }));
      ...
      document.getElementById("confirmSched").addEventListener("click", async ()=>{
        ...
        try {
          const res = await api("/api/appointments", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ doctor_id: docId, start_datetime: start_iso, notes })});
          toast("Booked");
  ...
  if (document.getElementById("apptsContainer")) {
    (async ()=>{
      ...
      async function render(){
        ...
        let filtered = appts.filter(a => a.status === "scheduled" || a.status === "Upcoming" ? a.status === "scheduled" : true);
        ...
        container.innerHTML = filtered.map(a => `
          <div class="card p-3 mb-3">
            <div class="d-flex justify-content-between">
              <div>
                <div class="fw-bold">Doctor #${a.doctor_id}</div>
